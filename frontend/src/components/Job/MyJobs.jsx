...
export const MyJobs = () => {
  ...
  const handleUpdateJob = async (jobId) => {
    ...
    try {
      const { data } = await axios.put(`http://localhost:8080/api/v1/job/update/${jobId}`, updatedJob, {
        withCredentials: true,
      });
  ...
  const handleDeleteJob = async (jobId) => {
    try {
      const { data } = await axios.delete(`http://localhost:8080/api/v1/job/delete/${jobId}`, {
        withCredentials: true,
      });
      toast.success(data.message);
